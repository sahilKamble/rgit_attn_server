...
attnRouter.route('/sub/:subid/:studid').get(isAuth, (req, res, next) => {
    Attendance.find({
...
attnRouter.route('/table/:subid').get(isAuth, (req, res, next) => {
    Subjects.findById(req.params.subid).exec((err, subject) => {
        if (err) next(err);
        Attendance.aggregate([
            {
                $match: {
                    student: { $in: subject.students },
                    subject: subject._id,
                },
            },
            { $sort: { date: 1 } },
