...
subjectRouter
    ...
    .post(isAuth, (req, res, next) => {
        res.statusCode = 403;
        res.end(
            'POST operation not supported on /subjects/' + req.params.subjectId
        );
    })
        ...
        Subjects.findByIdAndUpdate(
            ...
            .then((subject) =>
                subject
                    .model('User')
                    .updateMany(
                        { _id: { $in: subject.sharedWith } },
                        { $addToSet: { sharedSubjects: subject._id } }
                    )
                    .then(
                        (response) => {
                            res.statusCode = 200;
        ...
        Subjects.findByIdAndUpdate(req.params.subjectId, {
            ...
            .then((subject) =>
                subject
                    ...
                    .updateMany(
                        { _id: { $in: subject.sharedWith } },
                        { $pull: { sharedSubjects: subject._id } }
                    )
                    .then(
                        (response) => {
                            res.statusCode = 200;
